...
def load_joined_csv(joined_path: Path) -> List[Dict[str, Any]]:
    """
    Load joined.csv and normalise to a common schema:
    ...
    - New slim format: receiver, metric, band_hz, target, prediction
    - Old format: f_hz, target_val, predicted_val, error
    """
    ...
    with joined_path.open("r", encoding="utf-8", newline="") as f:
        ...
        for row in reader:
            if "f_hz" in fieldnames:
            ...
            elif "band_hz" in fieldnames:
                # New slim format: receiver, metric, band_hz, target, prediction
                band = float(row["band_hz"])
                ...
                rows.append({
                    ...
                    "target_val": target,
                    "predicted_val": pred,
                    "error": pred - target,  # consistent with detailed_results_log.json
                    "receiver": row.get("receiver"),
                    "metric": row.get("metric"),
                })

            else:
                raise RuntimeError(f"Unexpected joined.csv schema: {fieldnames}")
