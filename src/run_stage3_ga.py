...
def call_forward_and_get_loss(config_path: Path,
                              ...
                              project_root: Path,
                              timeout_s: int) -> float:
    """Run forward as a module from project_root; parse scalar Loss."""
    cmd = [
    ...
    # We run from project_root so src.* imports resolve robustly.
    # The forward itself writes outputs into paths defined by its own config (not CWD),
    # while we keep our per-eval files in `workdir`.
    try:
        proc = subprocess.run(
